...
from GCNL.data_utils import get_mlb, get_pid_list, output_res
...
@click.option('-n', 'num_models', type=click.INT, default=None)    # 6个模型
def main(data_cnf, model_cnf, num_models):
    ...
    for i in range(num_models):
        sc_mat += np.load(res_path/F'{model_name}-Model-{1}-{data_name}-Task-{i + 1}-{test_name}.npy') / num_models
