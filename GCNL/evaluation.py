...
def fmax(targets: ssp.csr_matrix, scores: np.ndarray):
    ...
    for cut in (c / 100 for c in range(101)):
        ...
        with warnings.catch_warnings():
            ...
            p, r = correct / cut_sc.sum(axis=1), correct / targets.sum(axis=1)   # 当前阈值下的准确率和召回率
            p, r = np.average(p[np.invert(np.isnan(p))]), np.average(r)   # 取平均值
        if np.isnan(p):
            continue
        try:
